...
class ErrorHandler
{
    ...
    public static function register(self $handler = null, $replace = true)
    {
        ...
        if (null === $prev = set_error_handler([
            $handler,
            'handleError'
        ])) {
        ...
        if (\is_array($prev = set_exception_handler([
            $handler,
            'handleException'
    ...
    public function handleError($type, $message, $file, $line)
    {
        ...
        if (null !== self::$toStringException) {
            ...
            if (! isset(self::$silencedErrorCache[$id = $file . ':' . $line])) {
                ...
                $errorAsException = new SilencedErrorContext($type, $file, $line, isset($lightTrace[1]) ? [
                    $lightTrace[0]
                ] : $lightTrace);
        ...
        if ($this->isRecursive) {
            ...
            try {
                ...
                $this->loggers[$type][0]->log($level, $logMessage, $errorAsException ? [
    ...
    public function handleException($exception, array $error = null)
    {
        ...
        if ($this->loggedErrors & $type) {
            try {
                $this->loggers[$type][0]->log($this->loggers[$type][1], $message, [
                    'exception' => $exception
                ]);
