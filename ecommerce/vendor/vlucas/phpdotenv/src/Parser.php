...
class Parser
{
    ...
    public static function parse($entry)
    {
        ...
        return [
            self::sanitiseName($name),
            self::sanitiseValue($value)
        ];
    }
    ...
    private static function sanitiseValue($value)
    {
        ...
        if (preg_match('/\s+/', $value) > 0) {
            ...
            if (preg_match('/^#/', $value) > 0) {
                ...
                throw new InvalidFileException(self::getErrorMessage('an unexpected space', $value));
            }
        }
    ...
    private static function processQuotedValue($value)
    {
        ...
        return Regex::replace($pattern, '$1', $value)->mapSuccess(function ($str) use ($quote) {
            return str_replace('\\\\', '\\', str_replace("\\$quote", $quote, $str));
        })
            ->mapError(function ($err) use ($value) {
            throw new InvalidFileException(self::getErrorMessage(sprintf('a quote parsing error (%s)', $err), $value));
        })
            ->getSuccess();
    }
