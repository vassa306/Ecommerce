...
trait Units
{
    ...
    public function addUnit($unit, $value = 1, $overflow = null)
    {
        ...
        if ($unit === 'weekday') {
            ...
            'month',
            'year'
        ]) && ($overflow === false || ($overflow === null && ($ucUnit = ucfirst($unit) . 's') && ! ($this->{'local' . $ucUnit . 'Overflow'} ?? static::{'shouldOverflow' . $ucUnit}())))) {
            $day = $date->day;
        }
